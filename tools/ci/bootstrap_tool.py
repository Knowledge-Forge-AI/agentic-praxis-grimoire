#!/usr/bin/env python3
"""Install one exact pre-review tool from the closed APGR manifest."""
...
def host_platform() -> str:
    ...
    if system is None or machine is None:
        raise RuntimeError(f"host platform ({platform.system()}-{platform.machine()}) has no closed pre-review tool asset")
    return f"{system}-{machine}"


def load_manifest(path: Path = MANIFEST_PATH) -> dict:
    document = json.loads(path.read_text(encoding="utf-8"))
    if document.get("schema") != "apg-pre-review-tools-v1":
