...
def catalog_text(
    ...
    *,
    ...
    rows = "\n".join(
        (
            f"| [`{name}`]("
            f"{'chatgpt/' if name in nested else ''}{name}/SKILL.md"
...
class APGCheckSkillLibraryTests(unittest.TestCase):
    ...
    def test_nested_chatgpt_leaf_passes_with_exact_catalog_and_projection(self) -> None:
        ...
        nested = self.root / "skills" / "chatgpt" / name
        nested.parent.mkdir()
        source.rename(nested)
        projection = self.root / ".agents" / "skills" / name
        projection.unlink()
        projection.symlink_to(
            Path("../../skills/chatgpt") / name,
            target_is_directory=True,
        )
        (self.root / "skills" / "README.md").write_text(
            catalog_text(self.names, nested=frozenset({name})),
            encoding="utf-8",
        )

        result = self.run_checker(self.root)
        self.assertEqual(result.returncode, 0, result)
        self.assertIn("2 canonical skills", result.stdout)

        (self.root / "skills" / "README.md").write_text(
            catalog_text(self.names),
            encoding="utf-8",
        )
    ...
    def test_catalog_structure_boundaries_are_checked_by_the_real_cli(self) -> None:
        readme = self.root / "skills" / "README.md"
        cases = (
            ("# APG Skill Library\n", "APG022"),
            ("# APG Skill Library\n\n## APG v0.1 catalog\n", "APG023"),
        ...
        for index, (content, expected_code) in enumerate(cases):
            with self.subTest(index=index):
                ...
                self.assertIn(expected_code, self.result_codes())

    def test_fragment_and_external_links_are_accepted_by_the_real_cli(self) -> None:
        self.replace_skill(
            "alpha-skill",
            "Evidence for common mistakes.",
            (
                "Evidence for common mistakes.\n\n"
                "[Local section](#alpha-skill) and "
    ...
    def test_support_entry_named_like_directory_must_be_a_directory(self) -> None:
        support = self.root / "skills" / "alpha-skill" / "scripts"
        support.write_text("not a directory\n", encoding="utf-8")
        self.assertIn("APG018", self.result_codes())
