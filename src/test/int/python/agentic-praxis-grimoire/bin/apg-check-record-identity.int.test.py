...
class APGRecordIdentityTests(unittest.TestCase):
    ...
    def test_real_cli_reports_malformed_paths_stale_index_and_availability(self) -> None:
        malformed = self.root / "docs/status/not-an-exit.md"
        malformed.write_text("# Not an exit\n", encoding="utf-8")
        index = self.root / "docs/status/README.md"
        index.write_text(
            index.read_text(encoding="utf-8")
            + "- [`99999 — APG999 Missing`](2026/07/21/99999-apg999-missing-exit.md)\n",
            encoding="utf-8",
        )
        result = self.run_checker(
            "--format",
        ...
        self.assertTrue({"APGR004", "APGR006", "APGR013"}.issubset(codes))

    def test_real_cli_rejects_invalid_phase_and_non_directory_root(self) -> None:
        invalid = self.run_checker("--expect-available", "not-a-phase")
        self.assertEqual(invalid.returncode, 2)
        root_file = Path(self.temporary.name) / "root-file"
        root_file.write_text("not a directory\n", encoding="utf-8")
        result = subprocess.run(
            [str(COMMAND), "--root", str(root_file)],
            check=False,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            text=True,
    ...
    def test_text_failure_rendering_is_exercised_by_real_cli(self) -> None:
        duplicate = self.root / "docs/adr/2026/07/0001-duplicate.md"
        duplicate.write_text("# 0001 — Duplicate\n", encoding="utf-8")
        result = self.run_checker()
        self.assertEqual(result.returncode, 1, result)
        self.assertIn("FAIL APG record identity", result.stdout)
        self.assertIn("diagnostic", result.stdout)

    def test_text_success_rendering_is_exercised_by_real_cli(self) -> None:
        result = self.run_checker()
        self.assertEqual(result.returncode, 0, result)
        self.assertIn("PASS APG record identity", result.stdout)

    def test_malformed_adr_paths_do_not_allocate_the_adr_namespace(self) -> None:
        malformed = self.root / "docs/adr/not-an-adr.md"
        malformed.write_text("# Not an ADR\n", encoding="utf-8")
        result = self.run_checker("--format", "json")
        self.assertEqual(result.returncode, 0, result)
        self.assertEqual(json.loads(result.stdout)["summary"]["adrs"], 1)

    def test_missing_heading_is_a_real_identity_disagreement(self) -> None:
        path = self.root / "docs/status/2026/07/21/00029-apg19a-identity-exit.md"
        path.write_text("Phase ID: `APG19A`\n", encoding="utf-8")
        self.assertIn("APGR010", self.codes())

    def test_duplicate_phase_fields_are_rejected_from_real_record_text(self) -> None:
        path = self.root / "docs/status/2026/07/21/00029-apg19a-identity-exit.md"
        path.write_text(
            "# APG19A Identity Exit\n\n"
            "Phase ID: `APG19A`\n\n"
