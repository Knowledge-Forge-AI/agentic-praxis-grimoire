...
COMMAND_SOURCE = REPOSITORY_ROOT / "bin" / "apg-project-skills"
CANONICAL_SOURCE = REPOSITORY_ROOT / "skills"
V0_2_SKILLS = (
...
class APGProjectSkillsTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        for helper in (
            ...
            "apg_project_skills_core.py",
        ):
            shutil.copy2(
                REPOSITORY_ROOT / "libexec" / helper,
                self.source / "libexec" / helper,
            )
        for skill in SKILLS:
            relative = (
                Path("chatgpt") / skill
                if skill in NESTED_SKILLS
                else Path(skill)
            )
            shutil.copytree(
        ...
        self.command = self.source / "bin" / "apg-project-skills"
    ...
    def run_command(
        ...
        *arguments: str,
        ...
        command = [
            ...
            "--branch",
            "--source",
            str(self.source / "libexec"),
            "--rcfile",
            str(REPOSITORY_ROOT / ".coveragerc"),
            str(self.command),
            *arguments,
        ]
    ...
    def create_manual_link(
        ...
        link.symlink_to(
            target or self.canonical_skill(skill),
            target_is_directory=True,
        )
        return link

    def canonical_skill(self, skill: str) -> Path:
        relative = (
            Path("chatgpt") / skill
            if skill in NESTED_SKILLS
            else Path(skill)
        )
        return self.canonical_root / relative
    ...
    def test_02_installs_all_twenty_eight_into_an_empty_worktree(self) -> None:
        ...
        self.assertEqual(
            Path(str(state["apg_root"])).resolve(), self.source.resolve()
        )
        ...
        for skill in SKILLS:
            ...
            self.assertEqual(link.resolve(), self.canonical_skill(skill).resolve())
    ...
    def test_nested_chatgpt_sources_work_for_default_and_explicit_install(
        self,
    ) -> None:
        selected = tuple(sorted(NESTED_SKILLS))
        arguments = [
            item for skill in selected for item in ("--skill", skill)
        ]
        self.assert_success(self.run_command("install", *arguments))
        for skill in selected:
            self.assertEqual(
                self.projection(skill).resolve(),
                self.canonical_skill(skill).resolve(),
            )
        self.assert_success(self.run_command("check", *arguments))
    ...
    def test_07_adopt_refuses_a_mismatched_link(self) -> None:
        self.create_manual_link(
            SKILLS[0], target=self.canonical_root / SKILLS[1]
        )
    ...
    def test_11_malformed_and_unsupported_state_are_refused(self) -> None:
        ...
        valid = {
            "format_version": 1,
            "apg_root": str(self.source.resolve()),
            "target_root": str(self.repo.resolve()),
            "managed_skills": [SKILLS[0]],
            "created_containers": [],
        ...
        for label, content in (
            ...
            ("extra schema key", json.dumps({**valid, "extra": True}).encode()),
            ("relative APG root", json.dumps({**valid, "apg_root": "relative"}).encode()),
            ("different APG root", json.dumps({**valid, "apg_root": "/different"}).encode()),
            ("relative target root", json.dumps({**valid, "target_root": "relative"}).encode()),
            ("different target root", json.dumps({**valid, "target_root": "/different"}).encode()),
            ("empty managed set", json.dumps({**valid, "managed_skills": []}).encode()),
            ("unknown managed skill", json.dumps({**valid, "managed_skills": ["unknown"]}).encode()),
            ("invalid containers", json.dumps({**valid, "created_containers": ["outside"]}).encode()),
            ("invalid separator", json.dumps({**valid, "exclude_separator_added": 1}).encode()),
        ):
    ...
    def test_12_check_detects_missing_and_tampered_managed_links(self) -> None:
        ...
        link.symlink_to(
            self.canonical_root / SKILLS[1], target_is_directory=True
        )
        result = self.run_command("check")
        self.assert_safety_failure(result, "exact canonical")
    ...
    def test_19_duplicate_and_malformed_exclude_blocks_are_refused(self) -> None:
        variants = {
            ...
            "malformed begin marker": "# BEGIN APG PROJECT SKILLS V1 extra\n",
            "malformed end marker": "# END APG PROJECT SKILLS V1 extra\n",
            "unsafe body": f"{BEGIN_MARKER}\noutside\n{END_MARKER}\n",
            "duplicate entry": (
                f"{BEGIN_MARKER}\n/.agents/skills/{SKILLS[0]}\n"
                f"/.agents/skills/{SKILLS[0]}\n{END_MARKER}\n"
            ),
            "unsorted entries": (
                f"{BEGIN_MARKER}\n/.agents/skills/{SKILLS[1]}\n"
                f"/.agents/skills/{SKILLS[0]}\n{END_MARKER}\n"
            ),
        }
    ...
    def test_install_and_adopt_leave_normal_git_status_clean(self) -> None:
        self.assert_success(self.run_command("install"))
