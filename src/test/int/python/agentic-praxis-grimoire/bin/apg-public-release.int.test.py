...
class APGPublicReleaseTests(unittest.TestCase):
    ...
    def make_source(self, path: Path) -> Path:
        ...
        for public_path in policy["required_projections"]:
            ...
            skill_path = next(
                ...
                for item in policy["required_skills"]
                if PurePosixPath(item).parent.name == name
            )
            target.symlink_to(
    ...
    def copy_source_with_policy(
        ...
        for relative in sorted(historical_paths - current_paths):
            target = repo / relative
            target.parent.mkdir(parents=True, exist_ok=True)
            if relative in policy["required_wrappers"]:
                target.write_text("#!/bin/sh\nexit 0\n")
                target.chmod(0o755)
            elif relative in policy["required_test_entrypoints"]:
                if target.suffix == ".bats":
                    target.write_text('#!/usr/bin/env bats\n@test "fixture" { true; }\n')
                else:
                    target.write_text("print('ok')\n")
            else:
                target.write_text("# historical fixture owner\n")
        for relative in policy["required_projections"]:
            target = repo / relative
    ...
    def test_22_index_flags_cannot_mask_a_failing_configured_test(self) -> None:
        path = "src/test/unit/python/agentic-praxis-grimoire/libexec/apg_public_release.unit.test.py"
    ...
    def test_32_caller_environment_cannot_disable_configured_validation(self) -> None:
        ...
        self.commit_all(source, "Add failing configured test")
        candidate, built = self.build(
            self.root / "spoofed-configured-test-candidate", source
        )
        self.assert_success(built)
        for name, value in (
            ("APG_CONFIGURED_TESTS_ACTIVE", "1"),
