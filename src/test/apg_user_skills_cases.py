...
class APGUserSkillsCaseMixin:
    ...
    def test_policy_skill_and_projection_sets_must_agree(self) -> None:
        nested = "composing-approved-roadmap-assignments"
        names = tuple(
            sorted((*user_skills.SKILLS, "additional-profile", nested))
        )
        policy = {
            "required_skills": sorted(
                (
                    f"skills/chatgpt/{name}/SKILL.md"
                    if name == nested
        ...
        self.assertEqual(
            user_skills.policy_skill_paths(policy)[nested],
            Path("skills/chatgpt/composing-approved-roadmap-assignments"),
        )
