...
def _validate_sdist(path: Path, *, version: str) -> dict[str, Any]:
    ...
    if _version_tuple(version) >= (0, 11, 0):
        notes_file = f"release/v{version}-notes.md" if _version_tuple(version) >= (0, 12, 0) else "release/v0.11.0-notes.md"
        required.update(f"{expected_root}/{relative}" for relative in (
            ...
            "docs/distribution.md", "docs/public-pr-ci.md", notes_file,
        ))
...
def _validate_npm_package(
    ...
    *,
    ...
    if _version_tuple(version) >= (0, 8, 1) or "package/README.md" in contents:
